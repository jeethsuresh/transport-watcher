...
describe('ui markup', () => {
  it('renders the map title panel beside the menu button', () => {
    ...
    assert.match(page, /className="map-title-panel__title">Awesome TTC Watcher</);
    assert.doesNotMatch(page, /sidebar__head-row[\s\S]*className="title">Awesome TTC Watcher</);
  });
});
